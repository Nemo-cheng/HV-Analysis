...
description: |
  ...
  不要用于简单的名词解释（用户只是问"XX是什么"）、不要用于纯标题摘要生成。
---
...
**优先检查**：用户是否提供了已分析好的文件路径或内容？

- **是 → 直接模式**：读取用户提供的文件，跳过第一步（联网收集）、第二步（纵向分析）、第三步（横向分析）、第四步（横纵交汇），**直接进入第五步**（生成PDF）。读取文件内容后，仅做以下处理：
  1. 确认文件已完整包含报告结构（纵向、横向、交汇三部分）
  2. 若有明显格式问题（标题层级错乱、Markdown语法错误），修正后再转PDF
  3. 若内容完整且格式正常，直接转PDF，不做任何内容改动

- **否 → 标准模式**：继续往下执行，从第一步开始。
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要在「研究报告的严谨」和「可读性」之间找到平衡点。
...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "Nemo"
   ```
...
PDF 交付后，将报告及其拆解内容归档到个人知识库。

### 环境检测与初始化（首次使用必跑，后续自动跳过）

**步骤 A：读取 wiki 根目录配置**

检查 skill 目录下是否存在 `wiki-config.json`（路径：`<SKILL_DIR>/wiki-config.json`）：

- **存在**：读取其中的 `wiki_root` 字段，作为个人知识库根目录（下文称 `WIKI_ROOT`）
- **不存在**：
  1. 询问用户：「请输入你的知识库根目录路径（例：D:\自己学 或 ~/wiki）：」
  2. 将用户输入保存到 `<SKILL_DIR>/wiki-config.json`：
     ```json
     { "wiki_root": "<用户输入的路径>" }
     ```
  3. 进入步骤 B

**步骤 B：检测 wiki 框架是否已初始化**

检查 `<WIKI_ROOT>/TheSchema.md` 是否存在：

- **存在**：wiki 框架已就绪，跳过初始化，直接进入「写入路径判断」
- **不存在**：从 skill 自带的 `wiki-template/` 脚手架创建完整目录结构：
...
template_dir = skill_dir / "wiki-template"
wiki_root = Path(json.loads((skill_dir / "wiki-config.json").read_text())["wiki_root"])

# 复制模板文件和目录结构
shutil.copytree(template_dir, wiki_root, dirs_exist_ok=True)
print(f"✅ 知识库框架已初始化：{wiki_root}")
```

初始化完成后，提示用户：
> 「已在 `<WIKI_ROOT>` 初始化知识库框架（TheSchema.md、wiki/index.md、wiki/log.md 等）。首次使用建议打开 README.md 了解目录结构。」

> 个人知识库的结构和操作规范见 `<WIKI_ROOT>/TheSchema.md`（每次归档前必读）。
...
1. 获取当前工作目录的绝对路径（`cwd`）
2. 个人知识库根目录为 `WIKI_ROOT`（从 `wiki-config.json` 读取）
3. 判断逻辑：
   - 若 `cwd` 等于 `WIKI_ROOT` 或 `cwd` 是 `WIKI_ROOT` 的子目录 → **单写模式**：只写入当前目录
   - 否则 → **双写模式**：写入当前工作目录 **且** 同时写入 `WIKI_ROOT`

```python
from pathlib import Path
personal_wiki = Path(wiki_root).resolve()
cwd = Path.cwd().resolve()
is_inside_wiki = (cwd == personal_wiki or personal_wiki in cwd.parents)
# is_inside_wiki == True  →  单写模式
# is_inside_wiki == False →  双写模式
```

### 执行步骤

以下步骤均针对目标知识库目录（`WIKI_ROOT`，双写模式下还有当前项目的 wiki/）执行：

**步骤 1：从报告提取归档元信息**

读取 Markdown 报告，提取：
- `subject`：报告标题中的研究对象名称（如 `Harness Engineering`）
- `subject_type`：来自报告头部 `> 研究对象类型：` 字段（技术概念 / 产品 / 公司 / 人物）
- `domain`：来自报告头部 `> 所属领域：` 字段
- `summary`：来自报告「一、一句话定义」部分，提炼为 30 字以内独立可读的一句话
- `concepts_mentioned`：按 TheSchema § 9.1 规则提取（5-10 个，来自横向分析直接对比的概念）
- `entities_mentioned`：按 TheSchema § 9.1 规则提取（10-15 个，在纵向或横向中扮演实质角色的实体）

**步骤 2：写入报告页**

文件路径：`raw/comparisons/[研究对象]_横纵分析.md`

在报告正文最前面加 frontmatter（正文内容不得改动）：

```yaml
---
type: comparison
title: "[研究对象] 横纵分析"
summary: "[提炼自一句话定义，30字以内]"
subject: "[研究对象名称]"
...
对 `concepts_mentioned` 中每个概念，遵循 TheSchema § 9.2（关系图谱写法）和 § 9.3（渐进丰富规则）：

- 若 `wiki/concepts/[概念名].md` **已存在**：
  1. 在 `appears_in` 列表追加本报告链接
  2. 在「出现在哪些分析里」表格追加一行
  3. 若已有 2 份报告，可补充关系图谱中从本报告新获得的关系（必须附 WHY 说明）
  4. 更新 `updated` 字段

- 若**不存在**：创建新页面，包含：
  - frontmatter（含 `status/draft`）
  - 从报告中提炼的一句话定义
  - 关系图谱：只填报告中有据可查的关系，每条必须说明原因（见 TheSchema § 9.2 好写法示例）
  - `appears_in` 第一条
  - 「出现在哪些分析里」表格第一行
  - **不写**跨报告观察（只有 1 份来源时禁止写）

**步骤 4：处理实体页（entities/）**

对 `entities_mentioned` 中每个实体，同样分已存在/不存在两条路：

- **已存在**：追加 `appears_in` 条目和「在各分析中的角色」表格行，更新 `updated`
- **不存在**：创建新实体页，填写基本信息、简介（来自报告）、`appears_in` 第一条、角色表格第一行

**步骤 5：更新 index.md**

在 `wiki/index.md` 的「按领域浏览（raw/comparisons/）」下，找到或新建对应 `domain` 的分组，追加报告条目：

```markdown
| [[raw/comparisons/[研究对象]_横纵分析]] | [subject_type] | [summary] | [日期] |
```
...
扫描 `raw/comparisons/` 目录，统计与本报告 `domain` 字段相同的报告数量。

- 若达到 **3 份**（含本次）：提示用户：
  > 「[domain] 下已有 N 份分析（[列出报告名]），建议创建知识地图。是否现在创建？」
- 未达到 3 份：跳过，不提示。
